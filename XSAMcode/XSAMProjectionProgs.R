...
SetBpars<-function(dataobj,n,type=1){
	#Extends natural mortality, catch mean weight, stock mean weight and proportion mature at age with n years
	# type=1: last years value
	# type=2: average of three recent years
	A<-dataobj$A
	minAge<-dataobj$minAge
	maxAge<-dataobj$maxAge
	tmppropMat<-dataobj$propMat[nrow(dataobj$propMat),paste(minAge:maxAge)]
...
OneSampleApproach<-function(dataobj,fitobj,parobj){
	...
	cstdres<-cres[,1:ncol(vmat)]/sqrt(vmat)
