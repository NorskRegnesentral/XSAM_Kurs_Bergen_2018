ReadXSAMsettings<-function(file,CheckFile=T){
	#Reads XSAM configuration file. See "Documentation" p 5 for format of file. 
	#required variables
	variables<-c(
	"agerange",
	"yearrange",
...
CreateDataObject<-function(settings=XSAMsettings,datalist,deltaC=0,deltaI=0,NAval=NULL,actualAge=T){
	...
	#settings: XSAM configuration. Output from the function 'ReadXSAMsettings'
	#datalist:
	#	List containing all data to be used for fitting the model. The list must include the following objects:
	#	caa: 2 dimensional numeric object with catch at age (column) by year (row). The column names must be
	#	the actual age and row names the actual year. In addition it must contain the numerical attributes ages, years and a scaling factor k. The scaling factor k is multiplied to the catch at age numbers before the data enters the C++ template when this object is passed to the function “CreateDataObject”.
	#	SurveyIndices: List where each element holds the data for one survey. Each object in the list is a 2
	#	dimensional numeric object with survey indices at age (column) by year (row). The column names must be the actual age and row names the actual year. In addition it must contain the numerical attributes ages, years and a scaling factor k. The scaling factor k is multiplied to the respective numbers before the data enters the C++ template when this object is passed to the function “CreateDataObject”.
	...
	#	west: 2 dimensional numeric object with weight at age (column) by year (row) in stock. The column
	#	names must be the actual age and row names the actual year. In addition it must contain the numerical attributes ages and years.
	#	weca: 2 dimensional numeric object with weight at age (column) by year (row) in catch. The column
	#	names must be the actual age and row names the actual year. In addition it must contain the numerical attributes ages and years.
	#	caton: 2 dimensional numeric object with total catch (1 column) by year (row). The row names must be
	#	the actual year. In addition it must contain the numerical attributes years and a scaling factor k. The scaling factor k is multiplied to the respective numbers before the data enters the C++ template when this object is passed to the function “CreateDataObject”.
	#	natMor: 2 dimensional numeric object with natural mortality at age (column) by year (row) in catch. The
	#	column names must be the actual age and row names the actual year. In addition it must contain the numerical attributes ages and years.
	#	propF: 2 dimensional numeric object with proportion of F before spawning at age (column) by year (row)
	#	in catch. The column names must be the actual age and row names the actual year. In addition it must contain the numerical attributes ages and years.
	#	propM: 2 dimensional numeric object with proportion of M before spawning at age (column) by year
	#	(row) in catch. The column names must be the actual age and row names the actual year. In addition it must contain the numerical attributes ages and years.
	#	CatchPrediction: 2 dimensional numeric object with prediction of total catch in the assessment year with
	...
	#NAval: a value to be interpreted as NA, set to -999 by default
	#actualAge: if not T, then ages are interpreted as indices and not actual ages
	#
	#All matrices must be data.frame with ages as column names and years as row names
	#Returns: See "documentation" pp 2-4.
	#----------------------------------------------------------------------------------------------------------------------#
...
CreateParameterList<-function(dataobj,mapnames=NULL,sv=0){
	#Create ordering and dimension of parameters depending on which parameters are urned on or off
	#Set starting values to the parameters before optimizing
	#dataobj is output from 'CreateDataObject'
	#mapnames: if setting other parameters as known constants. Must be a name corresponding to ParameterNames (see below).
	#
	#Returns:
	#ParameterList which is a list with objects
	#	FixedPars: vector of parameters as in ParameterNames
	#	LP: matrix of latent variables
	#MapVector: vector which contains NA value for parameters which are set to a constant value (see map argument in MakeADFun)
	#ParameterNames: Names of parameters that are estimated depending on the configuration
	#-----------------------------------------------#
...
FitXSAMFunction<-function(dataobj,pars,control=list(eval.max=200,iter.max=150),lower=-Inf,upper=Inf,constraints=NULL,includeDiagnostics=T,returnObj=F){
	#fits the model using nlminb, create sdreports and return other pieces to be used for results and diagnostics
	#dataobj: output from CreateDataObject
	#pars: output from CreateParameterList 
	#control: control for 'nlminb'
	#lower and upper: as ion 'nlminb'
	#constraints: list with parameternames matching ParameterNames in Pars with lower and upper bounds
	#IncludeDiagnostics: if T, do TMBS 'sdreport' and extracts other estimates such as N, F, SSB etc... and Extracts residuals.
	#returnObj: if T returns the entire object created by MakeADFun such that any other statistics can be extracted after the fitting.
...
RunXSAMRetro<-function(XSAMdatalist,XSAMsettings,n,constraints=NULL,control=NULL,sdlist=NULL,StartValFixed=NULL){
	#Do a retro run
	#XSAMdatalist: as datalist, input to CreateDataObject
	#XSAMsettings: output from ReadXSAMsettings
	#Number of years in the retro
	#constraints and control: see FitXSAMfunction
	#sdlist: must externally provide errorstructures. This is because the function CreateDataObject resets the error structures each time it is called.
	#startValFixed: Can optionally provide startingvales for the parameters. Does not appear to matter for time to convergence.
